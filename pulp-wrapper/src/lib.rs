...
impl<const CORES: usize, const BUF_LEN: usize> PulpWrapper<CORES, BUF_LEN> {
    ...
    pub unsafe fn run<C: StreamCipher + StreamCipherSeek + KeyIvInit>(
        ...
        // pi_cl_team_fork(CORES, Self::entry_point_nobuf::<C>, data);
    }

    extern "C" fn entry_point_nobuf<C: StreamCipher + StreamCipherSeek + KeyIvInit>(
        data: *mut cty::c_void,
    ) {
        unsafe {
            let CoreData {
            ...
            let core_len = len / CORES;
            // let slice = core::slice::from_raw_parts_mut(source.add(core_id*9882), 9882);
            let slice = core::slice::from_raw_parts_mut(source.add(core_id*core_len), core_len);
            
            cipher.seek(core_id*core_len);
    ...
    extern "C" fn entry_point<C: StreamCipher + StreamCipherSeek + KeyIvInit>(
        ...
        unsafe {
            ...
            //     _ => panic!("unsupported"),
            // };
            let mut buf = <DmaBuf<CORES, BUF_LEN>>::new_from_l2(source, l1_alloc);
